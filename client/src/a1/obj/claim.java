...
public class claim extends ObjectVisual {
    ...
    protected void Parse() {
        if ((term instanceof OtpErlangAtom) && (((OtpErlangAtom)term).atomValue().equals("none")))
            ...
            if ((t.elementAt(0) instanceof OtpErlangAtom) && (((OtpErlangAtom)t.elementAt(0)).atomValue().equals("obj_claim"))) {
                claim_owner = Utils.ErlangInt(t.elementAt(1));
                if (claim_owner == Player.CharID) {
                    // ищем клайм с таким овнером
                    if (Claims.claims.containsKey(claim_owner) && Claims.claims.get(claim_owner).object_id == id) {
