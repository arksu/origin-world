...
public class GUI_Control {
	...
	// читаем добавление контрола из сети
	public void NetReadAdd(Packet pkt) {
		int id = pkt.read_int();
		GUI_Control c = gui.getRemoteControl(id);
		boolean is_exist_window = false;
		if (c != null) {
			if (c instanceof GUI_Window) {
				is_exist_window = true;
				Log.info("already exist remote control on add! " + id);
			}
			else
				Main.GlobalError("already exist remote control on add! " + id);
		} else {
			this.id = id;
			c = this;
		}
		// удаляем всех детей
		while (c.last_child != null && c.last_child.id > 0) c.last_child.Unlink();
		int x = pkt.read_int();
		int y = pkt.read_int();
		if (!is_exist_window) c.SetPos(x, y);
		// читаем параметры контрола
		c.DoNetRead(pkt);
		// читаем и добавляем детей если они есть.
		int childs_count = pkt.read_int();
		while (childs_count > 0) {
			childs_count--; 
			String type = pkt.read_string_ascii();
			if (!GUI.HaveType(type))
				Main.GlobalError("havnt this type of control! <" + type + "> id="+id);
			GUI_Control child = GUI.GetType(type).create(c);
			child.NetReadAdd(pkt);
		}
	}
	
	// читаем апдейт контрола из сети
	public void NetReadUpdate(Packet pkt) {
		int x = pkt.read_int();
		int y = pkt.read_int();
		SetPos(x, y);
		// читаем параметры контрола
