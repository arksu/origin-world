...
public class GUI_ListBox extends GUI_ScrollPage {
	...
	public GUI_ListBox(GUI_Control parent) {
		...
		min_size = new Coord(getSkin().GetElementSize(skin_element));
		SetStyle(true, false);
		ResetSelected();
	}
	...
	public boolean DoMouseBtn(int btn, boolean down) {
		...
		for (int i=0; i<GetCount(); i++) {
			...
			if ((ay+h >= abs_pos.y+ClientRect.y) && (ay < abs_pos.y+ClientRect.y+wr.h)) {
				...
				mouse_captured = gui.MouseInRect(new Coord(ax,ay), new Coord(wr.w,h));
	...
	protected void DrawItemBg(int index, int x, int y, int w, int h) {
		...
		if (GetSelected() == index) {
			if (gui.MouseInRect(new Coord(x, y), new Coord(w, h)) && MouseInMe())
	...
	protected void OnDeleteItem(int index) {
		...
		if (GetCount() <= SelectedItem) SelectedItem = GetCount()-1;
		UpdateFullSize();
	}
