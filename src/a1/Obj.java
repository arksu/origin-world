...
public class Obj {
    ...
    public Coord get_step_offset() {
        if (obj_type.equals("player")) {
            ...
            } else if (tile == MapCache.TILE_HOLE) {
                return new Coord(0, 10);
            }
        } else {
            if (follow_id != 0) {
...
//			Log.info("delete attr "+c.getCanonicalName());
    ...
    public void fill_follow_params(int index, Obj f) {
        ...
        // игрок тащит над собой что-то
        // при этом можно задавать особые оффсеты объектам которые тащим над собой. каждому индивидуально.
        if ("player".equals(obj_type)) {
            f.follow_addz = 20;
            f.follow_offset = new Coord(0, -40);
        }

        // скорректируем положение игрока на стуле
        if ("chair".equals(obj_type)) {
            switch (direction) {
                case 4: f.follow_offset = new Coord(-1, 0); break;
                case 1: f.follow_offset = new Coord(-3, -1); break;
                case 7: f.follow_offset = new Coord(2, -1); break;
                case 0: f.follow_offset = new Coord(-1, 0); break;
                default: f.follow_offset = Coord.z;
            }
        }
    }
    // end FOLLOW -------------------------------
